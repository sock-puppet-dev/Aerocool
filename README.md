...
Для системы отзывов подключен `Netlify Database`. Целевая архитектура описана в [docs/deploy/17-netlify-database-reviews.md](/Users/stadnyk/MEGA/Aerocool/docs/deploy/17-netlify-database-reviews.md): отзывы хранятся в PostgreSQL, проходят модерацию, выгружаются в `data/generated/reviews.json` на этапе build и только после этого попадают в видимый HTML и `Product` JSON-LD. На текущем этапе создана первая миграция `reviews`, добавлен `POST /api/reviews` и локально проверена запись `pending` отзыва для тестового товара `SKY Lite`.
...
Папка `netlify/database/migrations/` уже используется для review-системы: первая миграция создает таблицу `reviews`. Папка `data/generated/` появится позже, после реализации build-time export approved отзывов.
...
3. [docs/01-documentation-map.md](/Users/stadnyk/MEGA/Aerocool/docs/01-documentation-map.md) — полная карта документации и порядок чтения `01-55`.
