...
- `assets/css/tailwind.css`: generated by the Tailwind CLI scripts; do not hand-edit it.
...
- Start local development with `./script_start.sh`.
- The script runs `npm run dev` for Tailwind in the background and `hugo server` in the foreground.
- If you need separate processes, run `npm run dev` and `hugo server` manually.
- Netlify production build is `git submodule update --init --recursive && npm run build && hugo --gc --minify`.
