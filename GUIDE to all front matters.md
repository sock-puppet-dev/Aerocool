...
- Если в теле страницы нужно выделить точное техническое обозначение, характеристику или поисковую фразу через inline-code, использовать жирный code-формат: ``**`11D`**``, ``**`SYNC5 multi-adjustable`**``, ``**`офисное кресло`**``.
- Не переносить это правило на шаблонный код Hugo/JS в `layouts/`: там обратные кавычки являются частью кода.
