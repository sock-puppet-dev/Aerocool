...
- В теле товарной страницы показывать коммерческие данные, которые используются в `Product` JSON-LD: цену, SKU, гарантию, наличие и рейтинг, если он заполнен.
...
- Merchant-условия, которые попадают в `Product` JSON-LD, должны иметь видимое подтверждение в `/faq/`: доставка `Новой Почтой`, доставка для покупателя `0 грн`, передача в отправку `0-1 день`, транзит `1-3 дня`, возврат `14 дней`, бесплатный возврат и способы оплаты наличными/картой. Если бизнес-условия меняются, обновлять FAQ, product schema и текст товарных страниц одним пакетом.
