...
- Рейтинг: ... # только если подтвержден видимыми данными
...
- Front matter товарной страницы — единый источник правды для `Product` JSON-LD: цена, SKU, MPN, GTIN, гарантия, наличие, доставка, возврат, способы оплаты и рейтинг.
- В теле товарной страницы показывать коммерческие данные, которые используются в `Product` JSON-LD: цену, SKU, гарантию, наличие, delivery/return/payment facts и рейтинг, если он заполнен.
