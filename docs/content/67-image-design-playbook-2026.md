...
8. Проверь изображение по ручному дефект-чеклисту ниже.
9. Запусти `npm run build`.
...
- проверка изображений остается ручной: размеры, форматы, front matter, crops, logo lockup и product gallery assets нужно сверять по чеклисту;
...
2. Заменить одинаковые product primary images официальными разными изображениями по каждому SKU/MPN/GTIN.
