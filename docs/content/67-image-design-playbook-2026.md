...
7. Оптимизируй файл, положи его в правильное место и обнови `image`, `cover.image`, `cover.alt` и `seo-image`, если это page bundle.
...
- логотип Aerocool в верхнем левом углу для обложек, category covers и fallback;
...
| Home hero | `assets/images/home-hero85.webp` | около **1024x1536**, WebP | Первый экран главной | Portrait product hero |
...
- Размер: **1536x1024**.
- Ratio: **3:2**.
- Первое видимое изображение в теле страницы выводить через `seo-image`.
...
- Ориентир: **1024x1536**.
- Сцена: портретный product hero, без заголовков и бейджей.
- Важно: фокус на кресле, безопасные поля для головы, колес и responsive crop.
...
Портретная hero-картинка:

```bash
magick input.png -auto-orient -resize 1024x1536^ -gravity center -extent 1024x1536 -strip -quality 82 -define webp:method=6 home-hero85.webp
```
...
```text
Create a 1024x1536 portrait-oriented premium high-tech hero image for Aerocool Ukraine. One real Aerocool ergonomic chair is the central hero object, full product visible with enough safe space around the wheels and headrest. Dark graphite futuristic studio, cold cyan rim light, subtle violet depth, reflective floor, premium product photography mood. The chair must carry integrated Aerocool branding on the headrest and seat material. Keep the composition clean for a website homepage hero, with no text, no badges and no people.
```
...
- [ ] Логотип Aerocool не обрезан, не искажен и не перегорел.
