...
"для этого проекта используй Hugo 0.163.0 и Node 24.16.0".
```
...
`Tailwind CSS 4.3` зафиксирован как npm-зависимость в `package.json` и точной версией `4.3.0` в `package-lock.json`. Начиная с Hugo `0.161.0`, Tailwind CSS CLI должен устанавливаться через npm; в текущем Hugo `0.163.0` это правило остается актуальным.
...
Если команды показывают `Hugo 0.163.0` и `Node 24.16.0`, всё хорошо.

Примечание на 2026-06-11: проект обновлен до pinned-версии `0.163.0` в `mise.toml` и `netlify.toml`. Node target остается `24.16.0`. Следующие обновления Hugo делать только отдельной задачей с повторной сборкой, Tailwind/CWV/SEO-проверками и обновлением этого документа.

Примечание на 2026-06-24: если `mise current` показывает предупреждение вида `hugo@0.163.0 is specified ... but not installed` или `node@24.16.0 is specified ... but not installed`, обычная команда `hugo version` может взять глобальную системную версию Hugo. Для этого проекта так делать нельзя: сначала запусти `mise install` или любую проверочную команду через `mise exec --`, чтобы `mise` установил pinned-версии.
...
- `mise current` показывает `hugo 0.163.0` и `node 24.16.0` без предупреждений `not installed`;
- `mise exec -- hugo version` показывает Hugo `0.163.0`;
...
| Версия Hugo не `0.163.0` | Подхватилась системная версия | Запустить через `mise exec -- hugo version`. |
