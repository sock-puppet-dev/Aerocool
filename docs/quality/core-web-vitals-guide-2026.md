# Core Web Vitals Guide 2026 Для Aerocool
...
- [quality/unlighthouse-site-audit.md](/Users/stadnyk/MEGA/Aerocool/docs/quality/unlighthouse-site-audit.md) — массовый Lighthouse-аудит по сайту.
- [quality/lighthouse-single-page-audit.md](/Users/stadnyk/MEGA/Aerocool/docs/quality/lighthouse-single-page-audit.md) — точечный Lighthouse-аудит одной страницы.
- [content/seo-image-shortcode.md](/Users/stadnyk/MEGA/Aerocool/docs/content/seo-image-shortcode.md) — правила изображений в page bundle.
- [seo/google-seo-audit-checklist-2026.md](/Users/stadnyk/MEGA/Aerocool/docs/seo/google-seo-audit-checklist-2026.md) — общий SEO-аудит для выхода в ТОП-1 Google.
- [seo/ssg-seo-checklist-2026.md](/Users/stadnyk/MEGA/Aerocool/docs/seo/ssg-seo-checklist-2026.md) — общий SSG SEO-чеклист.
- [deploy/netlify-routing.md](/Users/stadnyk/MEGA/Aerocool/docs/deploy/netlify-routing.md) — Netlify redirects, headers и cache notes.
...
## 2. Как CWV Связаны С ТОП-1 Google
...
Нельзя обещать ТОП-1 только из-за хороших CWV. Но в конкурентной выдаче медленная, дергающаяся или плохо отвечающая страница почти всегда проигрывает сильной странице с хорошим UX.

## 3. Field Data И Lab Data

Есть два типа данных.

| Тип | Где смотреть | Что означает |
|---|---|---|
| Field data | Search Console, PageSpeed Insights, Chrome UX Report | реальные пользователи, обычно 28-дневное окно |
| Lab data | Lighthouse, Unlighthouse, DevTools | контролируемый тест здесь и сейчас |

Для SEO важнее field data, но для исправлений удобнее lab data.

Правильный порядок:

1. В Search Console смотреть группы URL с плохими `LCP`, `INP`, `CLS`.
2. В PageSpeed Insights открыть конкретный URL и сравнить field data с lab data.
...
6. Проверить lab data сразу.
7. Проверить field data после накопления новых реальных данных.
...
| Resource load delay | браузер поздно начал грузить LCP-ресурс | preload, HTML order, CSS/JS blockers |
| Resource load duration | сам ресурс грузится долго | размер изображения, формат, CDN |
| Element render delay | ресурс уже есть, но элемент поздно рисуется | CSS, JS, fonts, layout |

### LCP Фиксы Для Aerocool
...
| LCP image поздно обнаруживается | Использовать `preload=true` только для главной картинки первого экрана |
| Слишком тяжелое изображение | Подготовить WebP/AVIF, правильные размеры, responsive `srcset` |
...
| Правила shortcode | `docs/content/seo-image-shortcode.md` |
...
| Netlify routing notes | `docs/deploy/netlify-routing.md` |
...
| View transitions тормозят | Проверить `assets/js/site.js` и `docs/architecture/browser-view-transitions.md` |
...
Если менялись `static/_redirects`, `netlify.toml` headers или 404/routing, дополнительно смотреть [deploy/netlify-routing.md](/Users/stadnyk/MEGA/Aerocool/docs/deploy/netlify-routing.md).
...
## 15. CWV Checklist Перед Релизом
...
| TTFB | ≤ 0.8 s |
| Images | responsive, sized, AVIF/WebP где возможно |
