...
### P0.3. Проверить Local Organization, Address И Размещение `sameAs`
...
3. Держать глобальные `sameAs` только в `global-organization.html` и `brand.html`.
4. Для local organization оставлять связь через `parentOrganization` и `brand`; добавлять local `sameAs` только если появятся отдельные официальные профили именно `Aerocool Ukraine`.
...
2. Определить, кто и как обновляет product front matter в рабочем процессе контента.
...
2. Проверить local organization facts: адрес, телефон, email и размещение глобальных `sameAs`.
