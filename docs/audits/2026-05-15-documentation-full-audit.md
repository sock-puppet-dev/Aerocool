...
- оглавление и маршруты чтения: `docs/README.md`;
...
- фактическая структура проекта: `content/`, `layouts/`, `assets/`, `static/`, `data/`, `unlighthouse/`;
- конфиги: `hugo.yaml`, `netlify.toml`, `mise.toml`, `package.json`, `unlighthouse/package.json`;
...
- сверка всех файлов `docs/**/*.md` с `docs/README.md`;
- сверка документации с `hugo.yaml`, `netlify.toml`, `mise.toml`, root `package.json`, `static/_redirects`, `layouts/_partials`, `layouts/_shortcodes` и `unlighthouse`;
...
- сверка entity URL в `docs/seo/entity-registry-2026.md` с фактическими `slug` и `data/entities.yaml`.
...
Два неучтенных файла: сам `docs/README.md` и `docs/audits/2026-05-13-content-image-cover-alt-audit.md`. Второй пункт закрыт в этом проходе.
...
- документация хорошо покрывает Hugo 0.161.0, Node 24, Tailwind CSS 4, Netlify, Unlighthouse, SEO, schema.org, entity registry, AI Search и Core Web Vitals;
- есть понятный путь новичка: `README.md` -> `docs/README.md` -> front matter -> template helpers -> Unlighthouse;
- active guides в основном совпадают с фактической структурой `layouts/`, `content/`, `static/_redirects` и `unlighthouse/`;
...
В `docs/seo/entity-registry-2026.md` часть строк в section 16/17 использовала folder names page bundle вместо публичных slugs:
...
### P1. `docs/README.md` не ссылался на один существующий аудит

Файл `docs/audits/2026-05-13-content-image-cover-alt-audit.md` существовал и был полезен как контрольный снимок по `image`, `cover`, `cover.alt`, markdown H1, inline-code и content image consistency, но отсутствовал в разделе `Audits`.
...
В `docs/content/content-seo-checklist-2026.md` ссылки вели на правильные файлы, но видимый текст называл их `front-matter.md` и `helpers.md`, хотя актуальные имена:

- `front-matter-reference.md`;
- `hugo-template-helpers.md`.
...
- конфликта между root `package.json`, `mise.toml`, `netlify.toml` и tooling docs по Hugo 0.161.0 / Node 24 / Tailwind CSS 4.
...
1. Добавить легкую автоматическую проверку документации: broken local links, docs not linked from `docs/README.md`, stale route aliases in entity docs.
2. После добавления variant navigation обновить `entity-registry-2026.md`, `ecommerce-structured-data-playbook-2026.md` и `json-ld-graph-audit-roadmap-2026.md`.
...
Документация проекта находится в хорошем рабочем состоянии: она не просто описывает файлы, а задает правила безопасной работы с SEO, schema.org, content, images, Netlify, Unlighthouse и entity graph.
...
- `data/entities.yaml` и `entity-registry-2026.md` должны оставаться согласованными;
