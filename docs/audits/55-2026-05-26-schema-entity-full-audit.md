...
| `sky-lite` | `confirmed` | `Product` | `Product` | 6 | 14 | 0 | 1 | OK |
...
Это корректное состояние. `ProductGroup` нельзя включать только потому, что в registry уже есть ID. Актуализация `2026-05-28`: видимый selector между цветовыми вариантами уже добавлен через `variant-swatches.html`, но сами ProductGroup entities еще остаются `planned`. Актуализация `2026-05-31`: singleton ProductGroup для SKY Lite, SKY 360, WING Mesh Black и XTAL Mesh Black удалены; эти страницы остаются самостоятельными `Product` внутри своих серий. Для `confirmed` статуса нужны:
