...
Это корректное состояние. `ProductGroup` нельзя включать только потому, что в registry уже есть ID. Актуализация `2026-05-28`: видимый selector между цветовыми вариантами уже добавлен через `variant-swatches.html`, но сами ProductGroup entities еще остаются `planned`. Для `confirmed` статуса нужны:
