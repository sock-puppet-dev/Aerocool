...
2. `rating.value` и `rating.count` остаются в product front matter и видимом контенте, но approved reviews pipeline еще не внедрен;
3. `ProductGroup` подготовлен, но остается `planned` и не выводится в graph;
4. `SKY 360` и `SKY Lite` не имеют подтвержденных `mpn` и `gtin13`;
5. нет регулярного Entity Performance Report, который связывает registry, страницы, GSC, AI Search и бизнес-сигналы.

Важное улучшение текущего состояния: `Product.aggregateRating` сейчас не выводится в rendered JSON-LD. Шаблон [product.html](/Users/stadnyk/MEGA/Aerocool/layouts/_partials/_schema/product.html) уже подавляет legacy front matter rating и ждет approved public reviews. Поэтому риск rating/reviews больше не является синтаксическим или rendered JSON-LD риском. Он остается контентным и операционным риском: на товарных страницах видимые рейтинги и front matter поля еще существуют без подключенного review source of truth.
...
| Product nodes with `aggregateRating` | `0` |
| Product nodes with `review` | `0` |
| `rating` in product front matter | `24` |
| Visible rating lines | `24` |
| `review_target_id` | `0` |
| `reviews_enabled` | `0` |
...
- visible rating/front matter rating нужно либо связать с approved reviews, либо убрать как неподтвержденный сигнал.

## 11. Ratings И Reviews

Текущий факт: `AggregateRating` уже не выводится в `Product` JSON-LD.

Это правильное техническое решение до появления реального review pipeline. Но задача еще не закрыта полностью, потому что:

- `rating.value` и `rating.count` остаются во front matter всех product pages;
- видимая строка рейтинга остается на всех product pages;
- `review_target_id` не добавлен;
- `reviews_enabled` не добавлен;
- `data/generated/reviews.json` отсутствует;
- approved public reviews пока не выводятся на товарных страницах.

Практический вывод: сейчас нет риска, что Google увидит неподтвержденный `AggregateRating` в JSON-LD. Но есть риск пользовательского и governance-несоответствия: на странице виден рейтинг, а источник рейтинга не объяснен полноценным review-layer.

Допустимые решения:

1. Внедрить review pipeline: `Netlify Database -> approved reviews -> data/generated/reviews.json -> visible reviews -> Product aggregateRating`.
2. Убрать legacy `rating` из front matter и видимого product content, если источник рейтингов не может быть подтвержден.
...
### P0/P1. Ratings/Reviews Governance

`AggregateRating` не выводится в JSON-LD, что снижает structured data риск. Но `rating` остается во front matter и видимом товарном контенте. Нужно принять бизнес-решение:

- либо внедрить approved reviews pipeline;
- либо убрать legacy rating из front matter и UI.
...
Не внедрять сейчас. Вернуться после production, review pipeline, ProductGroup, entity reporting и появления реальных бизнес-процессов/API.

## 17. Рекомендуемая Очередь Работ

1. Закрыть production gate: перевести сборку в production только после финальной проверки.
2. Решить ratings/reviews: approved reviews pipeline или удаление legacy rating из front matter и UI.
3. Подтвердить MPN/GTIN для `SKY 360` и `SKY Lite`.
4. Добавить видимую навигацию вариантов на product pages.
5. Перевести готовые ProductGroup entities из `planned` в `confirmed` и включить `ProductGroup` / `isVariantOf`.
