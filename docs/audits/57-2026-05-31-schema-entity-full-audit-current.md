...
3. внешние performance-поля Entity Performance Report остаются `pending-production`, потому что GSC, AI citations и business signals можно честно заполнить только после production/indexability gate;
...
Главный вывод: больше не нужно решать `ProductGroup`, duplicate registry ID, planned-activation, базовый `additionalProperty`, описание процесса product facts или создание Entity Performance Report. Эти задачи закрыты. Следующие реальные задачи: production/indexability gate, SKY identifiers, заполнение внешних GSC/AI/business-метрик после production, post-production AI Search baseline и image license metadata после юридического подтверждения.
