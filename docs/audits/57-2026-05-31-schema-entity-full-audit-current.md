...
Контекст: это текущий полный срез schema/entity-слоя проекта `Aerocool Ukraine` после обновления Entity Registry, активации `ProductGroup` для реальных WING/XTAL цветовых вариантов, добавления `Product.color` / `Product.additionalProperty`, формализации Product Facts process, создания Entity Performance Report, внедрения Image License Metadata и повторной сборки Hugo 0.162.0. Аудит проверяет rendered JSON-LD, [data/entities.yaml](/Users/stadnyk/MEGA/Aerocool/data/entities.yaml), front matter `content/**/*.md`, Product graph, `about_entities`, `mentions_entities`, `product_group_id`, `sameAs`, ImageObject metadata, reviews pipeline, breadcrumbs и текущий список проблем.
...
- [59-entity-performance-report-2026.md](/Users/stadnyk/MEGA/Aerocool/docs/seo/59-entity-performance-report-2026.md).
...
1. сайт все еще собирается в `development`, поэтому все `114` JSON-LD страниц получают `noindex,nofollow`;
...
- операционный процесс поддержки product facts формализован в [58-product-facts-maintenance-process-2026.md](/Users/stadnyk/MEGA/Aerocool/docs/seo/58-product-facts-maintenance-process-2026.md): роли, владельцы, front matter source of truth, `/faq/`, JSON-LD QA и post-deploy проверка.
...
- Image License Metadata внедрена: добавлены страницы `/image-license/` и `/ru/image-license/`, registry-сущность `image-license-policy`, общий helper [image-license-metadata.html](/Users/stadnyk/MEGA/Aerocool/layouts/_partials/_schema/image-license-metadata.html), а все `228` rendered `ImageObject` nodes получили `license`, `acquireLicensePage`, `creator`, `creditText` и `copyrightNotice`.
...
Все `114` страниц с JSON-LD сейчас получают `noindex,nofollow`, потому что Netlify/Hugo намеренно настроены на `development`. Перед SEO-запуском нужно переключить production и проверить `index,follow`, canonical, hreflang, sitemap, robots, headers, 404 и rich results eligibility на published URL.
...
### P1. Post-Production Entity Metrics
