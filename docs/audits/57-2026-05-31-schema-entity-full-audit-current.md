...
3. `Product.additionalProperty` еще не построен из видимых характеристик;
4. нет регулярного Entity Performance Report, который связывает registry, страницы, GSC, AI Search и бизнес-сигналы;
5. image license metadata и post-production AI Search baseline еще не внедрены.
...
| Product nodes with `additionalProperty` | `0` |

Product/Offer слой сильный: цена, валюта, наличие, SKU, seller, доставка, возврат, оплата, гарантия и `priceValidUntil` собираются из front matter. ProductGroup activation закрыт для реальных WING/XTAL вариантов. Открытые product gaps: SKY identifiers и `additionalProperty`.
...
3. Спроектировать `additionalProperty` из видимых характеристик.
4. Завести Entity Performance Report.
5. Описать операционный процесс поддержки product front matter.
6. После production собрать GSC, Product rich results, AI citations и entity-level baseline.
7. Вернуться к image license metadata только после юридического подтверждения.
8. Вернуться к agentic actions только после появления реальных action endpoints и владельцев процесса.
...
| Product structured data | `9.1 / 10` | Offer facts и ProductGroup сильные; открыты SKY identifiers и `additionalProperty` |
...
Главный вывод: больше не нужно решать `ProductGroup`, duplicate registry ID или planned-activation. Эти задачи закрыты. Следующие реальные задачи: production/indexability gate, SKY identifiers, `additionalProperty`, Entity Performance Report и операционный процесс поддержки product front matter.
