...
| Серия | `CollectionPage`, позже `ProductGroup` где оправдано | `mainEntity`, visible variant links |
...
## 9. Image License Metadata

Текущий проект уже создает `ImageObject` для primary image страницы. Но image license metadata пока не внедрена.

Поля, которые можно рассмотреть позже:
...
Правило: не добавлять эти свойства, пока команда не подтвердила права на изображения и страницу с условиями использования. Для товарных изображений важнее сначала сохранить crawlable visible images, корректный `alt`, `contentUrl`, `width`, `height` и соответствие изображения реальному товару.

Image license metadata относится к P2. Она полезна для Google Images, но не должна появляться в JSON-LD как неподтвержденное юридическое утверждение.
...
## 10. Что Добавлять В План Развития
