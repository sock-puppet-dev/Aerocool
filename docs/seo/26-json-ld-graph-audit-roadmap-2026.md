...
Актуальный полный schema/entity-срез зафиксирован в [55-2026-05-26-schema-entity-full-audit.md](/Users/stadnyk/MEGA/Aerocool/docs/audits/55-2026-05-26-schema-entity-full-audit.md).
...
### Качество Google Rich Results: `8.8/10`

Минус не за синтаксис и не за schema.org Validator. После контентных обновлений цена, SKU, гарантия, наличие и merchant facts выводятся видимо, а `Product.aggregateRating` сейчас подавлен до появления approved reviews. Оставшийся риск — legacy `rating.value` / `rating.count` во front matter и видимом товарном контенте: рейтинг виден, но источник реальных пользовательских отзывов еще не подключен как полноценный moderated review layer.
...
Сейчас в `Product` JSON-LD нет `aggregateRating` и `review`. Это осознанное безопасное состояние до внедрения approved reviews pipeline.
...
Статус `2026-05-26`: для рейтингов принято целевое решение `Netlify Database -> approved reviews -> build-time export -> data/generated/reviews.json -> visible reviews -> Product JSON-LD`. До внедрения этого pipeline `aggregateRating` не выводится, но legacy rating во front matter и UI остается governance risk.
...
Если рейтинг и отзывы пока не являются реальным пользовательским сигналом, не выводить `aggregateRating` и не усиливать review-layer. Текущий шаблон уже следует этому правилу: `AggregateRating` будет возвращен только после approved reviews pipeline.
...
Видимые характеристики уже есть в product content/front matter. Следующий шаг — аккуратно усилить `Product`:
...
Добавлять эти поля стоит только из видимой таблицы характеристик или явного блока спецификаций. Не переносить в `additionalProperty` маркетинговые фразы, которые не являются стабильными характеристиками модели.
...
2. Решить ratings/reviews governance: approved reviews pipeline или удаление legacy rating из front matter и UI.
3. Подтвердить MPN/GTIN для `SKY 360` и `SKY Lite`.
4. Добавить видимую навигацию вариантов.
5. Перевести готовые ProductGroup entities в `confirmed` и включить `ProductGroup` / `isVariantOf`.
6. Спроектировать `additionalProperty` из видимых характеристик.
7. Завести регулярный Entity Performance Report.
8. Подготовить image license metadata только после подтверждения прав на изображения.
9. Вести entity/schema изменения через [20-schema-markup-quality-checklist-2026.md](/Users/stadnyk/MEGA/Aerocool/docs/seo/20-schema-markup-quality-checklist-2026.md), [24-entities-knowledge-graph-playbook-2026.md](/Users/stadnyk/MEGA/Aerocool/docs/seo/24-entities-knowledge-graph-playbook-2026.md), [21-ecommerce-structured-data-playbook-2026.md](/Users/stadnyk/MEGA/Aerocool/docs/seo/21-ecommerce-structured-data-playbook-2026.md) и [55-2026-05-26-schema-entity-full-audit.md](/Users/stadnyk/MEGA/Aerocool/docs/audits/55-2026-05-26-schema-entity-full-audit.md).
